# Decap CMS Implementation Plan
## Portfolio Template - Self-Service Content Management

**Business Model:** Reusable service template for professional portfolio clients
**Goal:** Build once, clients manage forever (no ongoing developer updates)
**Technology:** Decap CMS + Netlify (free, self-hosted, Git-based)

---

## Executive Summary

**Template Features:**
- ✅ Client will edit all content themselves (non-technical user)
- ✅ All page content should be CMS-editable
- ✅ Image upload capability required
- ✅ Multiple videos with dynamic sections (hide empty sections)
- ✅ **Multi-language support (Primary/Secondary language)**
- ✅ Service model: replicable for future clients

**Recommended Solution:** Decap CMS + Netlify Identity
- Free forever (no recurring costs)
- User-friendly interface for non-developers
- Git-based (content stored in repository)
- Image upload via Decap media library
- **Bilingual content management (Language 1/Language 2)**
- Replicable template for future clients

---

## Multi-Language Architecture

### ✅ Implemented Features

**Language Support:**
- Language 1 (lang1) - Primary language
- Language 2 (lang2) - Secondary language
- Client-side language switching with localStorage persistence
- Separate CMS content files per language

**Technical Implementation:**

1. **Language Context** (`src/contexts/LanguageContext.tsx`)
   - Global language state management
   - Persists user's language choice in localStorage
   - Provides `language` and `setLanguage` to all components

2. **Translation System** (`src/translations/translations.ts`)
   - Centralized UI translations (buttons, labels, navigation)
   - Typed translation objects for type safety
   - Covers: navigation, common elements, page-specific text

3. **Language Switcher** (`src/components/LanguageSwitcher.tsx`)
   - Toggle button: "LANG1 | LANG2"
   - Visible on all pages
   - Immediate language switching

4. **Content Loading Hook** (`src/hooks/useContentData.ts`)
   - Automatically loads language-specific content files
   - Uses current language from context
   - Fetches `content/{page}-{language}.json`

**Content File Structure:**
```
content/
├── home-lang1.json          # Primary language home page
├── home-lang2.json          # Secondary language home page
├── about-lang1.json         # Primary language about page
├── about-lang2.json         # Secondary language about page
├── gallery-lang1.json       # Primary language gallery
├── gallery-lang2.json       # Secondary language gallery
├── media-lang1.json         # Primary language media page
├── media-lang2.json         # Secondary language media page
├── cv-lang1.json            # Primary language CV
├── cv-lang2.json            # Secondary language CV
├── contact-lang1.json       # Primary language contact
└── contact-lang2.json       # Secondary language contact
```

**CMS Configuration:**
- Each page has TWO CMS entries (Language 1 + Language 2)
- Client can edit both languages independently
- Changes to one language don't affect the other
- Toggle feature allows hiding CV download button per language

---

## Content Analysis - What Will Be CMS-Editable?

### 🏠 HomePage
**Template Content:**
- Name: "CLIENT NAME"
- Subtitle: "Professional Title • Specialty • Expertise"
- Bio paragraph 1: "Introduction text about the client..."
- Bio paragraph 2: "Additional introduction or tagline..."
- Hero background image: `/assets/hero-bg.jpg`
- Button text: "View Gallery", "View Portfolio"

**CMS Fields:**
- ✅ All text content editable
- ✅ Hero background image changeable
- ✅ Button text customizable

---

### 👤 AboutPage
**Template Content:**
- Title: "Client Name"
- Multiple bio paragraphs with professional history
- Background image: `/assets/about-bg.jpg`

**CMS Fields:**
- ✅ All bio paragraphs editable
- ✅ Background image changeable

---

### 📸 GalleryPage
**Template Content:**
- Intro text: "Professional photos for media use. Click to view."
- Array of images with metadata

**CMS Fields:**
- ✅ Add/remove/reorder photos
- ✅ Upload new images (web + high-res versions)
- ✅ Edit alt text, photographer, category per image
- ✅ Edit intro text

---

### 🎬 MediaPage
**Template Content:**
- Video embeds with captions
- External media links
- Showreel or demo videos

**CMS Fields:**
- ✅ Add multiple videos (YouTube, Vimeo, direct)
- ✅ Add multiple external links
- ✅ Dynamic sections (auto-hide empty sections)
- ✅ Edit all titles and button text

---

### 📄 CVPage
**Template Content:**
- Multiple CV sections (customizable)
- CV download file
- Download button text

**CMS Fields:**
- ✅ Edit all CV sections (markdown support)
- ✅ Upload new CV file
- ✅ Edit button text

---

### 📧 ContactPage
**Template Content:**
- Email address
- Social media links (Facebook, Instagram, LinkedIn, etc.)
- Related links or professional affiliations

**CMS Fields:**
- ✅ Edit email address
- ✅ Edit social media URLs
- ✅ Add/remove/edit related links

---

## Implementation Architecture

### File Structure

```
portfolio-template/
├── public/
│   ├── admin/
│   │   ├── index.html          # Decap CMS admin panel
│   │   └── config.yml          # CMS configuration
│   ├── assets/
│   │   └── hero-bg.jpg         # Protected hero background
│   ├── cv/
│   │   ├── cv-lang1.txt        # Primary language CV download
│   │   └── cv-lang2.txt        # Secondary language CV download
│   └── images/                 # Media uploads folder
│       ├── web/                # Optimized images
│       ├── downloads/          # High-res originals
│       └── uploads/            # CMS uploaded images
├── content/                     # CMS content files (JSON) - MULTI-LANGUAGE
│   ├── home-lang1.json         # Primary language
│   ├── home-lang2.json         # Secondary language
│   ├── about-lang1.json
│   ├── about-lang2.json
│   ├── gallery-lang1.json
│   ├── gallery-lang2.json
│   ├── media-lang1.json
│   ├── media-lang2.json
│   ├── cv-lang1.json
│   ├── cv-lang2.json
│   ├── contact-lang1.json
│   └── contact-lang2.json
├── src/
│   ├── components/
│   │   ├── Navigation.tsx
│   │   └── LanguageSwitcher.tsx  # Language toggle
│   ├── contexts/
│   │   └── LanguageContext.tsx   # Language state
│   ├── hooks/
│   │   ├── useContentData.ts     # Auto-loads by language
│   │   └── useTranslations.ts    # UI translations
│   ├── translations/
│   │   └── translations.ts       # UI text (multi-language)
│   └── pages/                    # All pages use translations
└── ...
```

### Content Schema Design

#### 1. HomePage (`content/home.json`)
```json
{
  "name": "CLIENT NAME",
  "subtitle": "Professional Title • Expertise • Specialty",
  "heroParagraph1": "Introduction paragraph about the client...",
  "heroParagraph2": "Additional tagline or brief description...",
  "heroImage": "/assets/hero-bg.jpg",
  "ctaButton1Text": "View Gallery",
  "ctaButton1Link": "#gallery",
  "ctaButton2Text": "View Portfolio",
  "ctaButton2Link": "#media"
}
```

#### 2. AboutPage (`content/about.json`)
```json
{
  "title": "Client Name",
  "bioParagraphs": [
    "First paragraph about professional background...",
    "Second paragraph highlighting key achievements...",
    "Third paragraph about specialties and skills...",
    "Fourth paragraph with additional professional information..."
  ],
  "backgroundImage": "/assets/about-bg.jpg"
}
```

#### 3. GalleryPage (`content/gallery.json`)
```json
{
  "introText": "High-resolution images for media use. Click to view full size.",
  "images": [
    {
      "src": "/images/web/photo-1.jpg",
      "downloadSrc": "/images/downloads/photo-1.jpg",
      "alt": "Client Name professional photo",
      "photographer": "Photographer Name",
      "category": "Professional"
    }
  ]
}
```

#### 4. MediaPage (`content/media.json`) - **Dynamic Sections**
```json
{
  "sections": [
    {
      "type": "videos",
      "title": "Video Portfolio",
      "videos": [
        {
          "type": "youtube",
          "url": "https://www.youtube.com/embed/VIDEO_ID",
          "title": "Video Title",
          "description": "Video description"
        }
      ]
    },
    {
      "type": "showreel",
      "title": "Showreel",
      "videos": []
    },
    {
      "type": "samples",
      "title": "Work Samples",
      "externalLinks": [
        {
          "url": "https://example.com/portfolio",
          "buttonText": "View Portfolio"
        }
      ]
    },
    {
      "type": "media-mentions",
      "title": "Media Appearances",
      "externalLinks": [
        {
          "url": "https://example.com/interview",
          "buttonText": "Read Interview"
        }
      ]
    }
  ]
}
```

**Key Feature:** Sections with empty `videos` or `externalLinks` arrays won't render!

#### 5. CVPage (`content/cv.json`)
```json
{
  "sections": [
    {
      "title": "Education",
      "content": "University degree, certifications..."
    },
    {
      "title": "Experience",
      "content": "Professional work history..."
    },
    {
      "title": "Skills",
      "content": "Professional skills and expertise..."
    },
    {
      "title": "Achievements",
      "content": "Awards, recognition, notable accomplishments..."
    }
  ],
  "cvFile": "/cv/cv.pdf",
  "downloadButtonText": "⬇ Download CV"
}
```

#### 6. ContactPage (`content/contact.json`)
```json
{
  "email": "contact@clientdomain.com",
  "socialMedia": [
    {
      "platform": "facebook",
      "url": "https://www.facebook.com/username"
    },
    {
      "platform": "instagram",
      "url": "https://www.instagram.com/username"
    },
    {
      "platform": "linkedin",
      "url": "https://www.linkedin.com/in/username"
    }
  ],
  "relatedLinks": [
    {
      "url": "https://www.example.com",
      "text": "www.example.com"
    },
    {
      "url": "https://www.professional-org.com",
      "text": "www.professional-org.com"
    },
    {
      "url": "https://www.association.com",
      "text": "www.association.com"
    }
  ]
}
```

---

## Decap CMS Configuration

### `public/admin/config.yml`

```yaml
backend:
  name: git-gateway
  branch: main

media_folder: "public/images/uploads"
public_folder: "/images/uploads"

collections:
  - name: "pages"
    label: "Pages"
    files:
      # HomePage
      - label: "Home Page"
        name: "home"
        file: "content/home.json"
        fields:
          - {label: "Name", name: "name", widget: "string"}
          - {label: "Subtitle", name: "subtitle", widget: "string"}
          - {label: "Hero Paragraph 1", name: "heroParagraph1", widget: "text"}
          - {label: "Hero Paragraph 2", name: "heroParagraph2", widget: "text"}
          - {label: "Hero Background Image", name: "heroImage", widget: "image"}
          - {label: "Button 1 Text", name: "ctaButton1Text", widget: "string"}
          - {label: "Button 1 Link", name: "ctaButton1Link", widget: "string"}
          - {label: "Button 2 Text", name: "ctaButton2Text", widget: "string"}
          - {label: "Button 2 Link", name: "ctaButton2Link", widget: "string"}

      # AboutPage
      - label: "About Page"
        name: "about"
        file: "content/about.json"
        fields:
          - {label: "Title", name: "title", widget: "string"}
          - {label: "Bio Paragraphs", name: "bioParagraphs", widget: "list", field: {label: "Paragraph", name: "paragraph", widget: "text"}}
          - {label: "Background Image", name: "backgroundImage", widget: "image"}

      # GalleryPage
      - label: "Gallery"
        name: "gallery"
        file: "content/gallery.json"
        fields:
          - {label: "Intro Text", name: "introText", widget: "text"}
          - label: "Images"
            name: "images"
            widget: "list"
            fields:
              - {label: "Image (Web)", name: "src", widget: "image"}
              - {label: "High-Res Image", name: "downloadSrc", widget: "image"}
              - {label: "Alt Text", name: "alt", widget: "string"}
              - {label: "Photographer", name: "photographer", widget: "string"}
              - {label: "Category", name: "category", widget: "select", options: ["Headshots", "Production", "Promotional"]}

      # MediaPage
      - label: "Media Page"
        name: "media"
        file: "content/media.json"
        fields:
          - label: "Sections"
            name: "sections"
            widget: "list"
            fields:
              - {label: "Section Type", name: "type", widget: "string"}
              - {label: "Section Title", name: "title", widget: "string"}
              - label: "Videos"
                name: "videos"
                widget: "list"
                required: false
                fields:
                  - {label: "Video Type", name: "type", widget: "select", options: ["youtube", "vimeo", "direct"]}
                  - {label: "Video URL", name: "url", widget: "string"}
                  - {label: "Title/Caption", name: "title", widget: "string"}
                  - {label: "Description", name: "description", widget: "text", required: false}
              - label: "External Links"
                name: "externalLinks"
                widget: "list"
                required: false
                fields:
                  - {label: "URL", name: "url", widget: "string"}
                  - {label: "Button Text", name: "buttonText", widget: "string"}

      # CVPage
      - label: "CV Page"
        name: "cv"
        file: "content/cv.json"
        fields:
          - label: "CV Sections"
            name: "sections"
            widget: "list"
            fields:
              - {label: "Section Title", name: "title", widget: "string"}
              - {label: "Content", name: "content", widget: "markdown"}
          - {label: "CV File", name: "cvFile", widget: "file"}
          - {label: "Download Button Text", name: "downloadButtonText", widget: "string"}

      # ContactPage
      - label: "Contact Page"
        name: "contact"
        file: "content/contact.json"
        fields:
          - {label: "Email", name: "email", widget: "string"}
          - label: "Social Media"
            name: "socialMedia"
            widget: "list"
            fields:
              - {label: "Platform", name: "platform", widget: "select", options: ["facebook", "instagram", "youtube", "twitter", "linkedin"]}
              - {label: "URL", name: "url", widget: "string"}
          - label: "Related Links"
            name: "relatedLinks"
            widget: "list"
            fields:
              - {label: "URL", name: "url", widget: "string"}
              - {label: "Display Text", name: "text", widget: "string"}
```

---

## Implementation Steps

### Phase 1: Setup Decap CMS (1-2 hours)

1. **Install Decap CMS package**
   ```bash
   npm install decap-cms-app
   ```

2. **Create admin panel files**
   - Create `/public/admin/index.html`
   - Create `/public/admin/config.yml` (use config above)

3. **Create content folder**
   - Create `/content/` directory
   - Add JSON files with current hardcoded content

4. **Test locally**
   - Run `npm run dev`
   - Visit `http://localhost:5173/admin`
   - Test CMS interface (local mode, no auth needed)

### Phase 2: Refactor Components (3-4 hours)

1. **Create content loading hook** (`src/hooks/useContentData.ts`)
   ```typescript
   import { useState, useEffect } from 'react';

   export function useContentData<T>(contentFile: string): T | null {
     const [data, setData] = useState<T | null>(null);

     useEffect(() => {
       fetch(`/content/${contentFile}`)
         .then(res => res.json())
         .then(setData)
         .catch(console.error);
     }, [contentFile]);

     return data;
   }
   ```

2. **Refactor each page to use JSON data**
   - HomePage: Read from `home.json`
   - AboutPage: Read from `about.json`
   - GalleryPage: Read from `gallery.json`
   - MediaPage: Read from `media.json` + **implement conditional rendering**
   - CVPage: Read from `cv.json`
   - ContactPage: Read from `contact.json`

3. **Implement dynamic sections for MediaPage**
   ```typescript
   {data.sections.map((section) => {
     // Skip sections with no content
     if (section.videos && section.videos.length === 0 &&
         section.externalLinks && section.externalLinks.length === 0) {
       return null;
     }

     return (
       <div key={section.type}>
         <h2>{section.title}</h2>
         {/* Render videos or links */}
       </div>
     );
   })}
   ```

### Phase 3: Deploy to Netlify (1 hour)

1. **Push to GitHub**
   ```bash
   git add .
   git commit -m "Add Decap CMS integration"
   git push origin main
   ```

2. **Deploy to Netlify**
   - Connect GitHub repository
   - Set build command: `npm run build`
   - Set publish directory: `dist`
   - Deploy

3. **Enable Netlify Identity**
   - Go to Site Settings → Identity
   - Enable Identity
   - Enable Git Gateway
   - Invite client as user (they'll receive email)

4. **Test CMS on production**
   - Visit `https://clientdomain.netlify.app/admin`
   - Client logs in with Netlify Identity
   - Make test edit, verify it creates Git commit

### Phase 4: Training (30 mins)

1. **Create simple guide for client:**
   - How to log in (`/admin`)
   - How to edit text
   - How to upload images
   - How to add/remove gallery photos
   - How to add new video sections
   - **How to edit both language versions** (each page has 2 entries)
   - How to toggle CV download button on/off
   - How changes are saved (creates commits)
   - How to publish changes (auto-deployed)

2. **Walk through with client**
   - Show live editing
   - Demonstrate image upload
   - **Show language switching on live site**
   - **Show how to edit both language versions**
   - Explain publish flow

---

## Benefits of This Approach

### For You (Service Provider)
- ✅ **Replicable template** - Clone for next client in any industry
- ✅ **No ongoing maintenance** - Clients update themselves
- ✅ **Proven CMS solution** - Skill useful for future projects
- ✅ **Free infrastructure** - No CMS hosting costs to pass on

### For Client
- ✅ **Full control** - Update any content anytime
- ✅ **User-friendly** - No code knowledge needed
- ✅ **Image management** - Upload photos directly
- ✅ **Bilingual site** - Reach multiple language audiences
- ✅ **Easy language management** - Edit both languages separately
- ✅ **Immediate updates** - Changes go live in minutes
- ✅ **No recurring costs** - Free forever

### Technical Benefits
- ✅ **Git-based** - Full version history, can rollback changes
- ✅ **Static site** - Fast, secure, cheap to host
- ✅ **No database** - Content stored as JSON in repository
- ✅ **Multi-language architecture** - Clean separation of language content
- ✅ **Client-side language switching** - Instant language changes, no page reload
- ✅ **Automatic deploys** - Netlify rebuilds on every edit

---

## Future Enhancements (Optional)

1. **Netlify Large Media**
   - Automatic image optimization
   - Git LFS for large files
   - Multiple image sizes generated automatically

2. **Scheduled Publishing**
   - Editorial workflow (draft → review → publish)
   - Scheduled content releases

3. ✅ **Localization** - IMPLEMENTED
   - ✅ Multi-language versions of site
   - ✅ CMS manages both languages content
   - ✅ Language switcher component
   - ✅ Separate content files per language

4. **Additional Languages** (Future)
   - Easy to add third, fourth languages
   - Same pattern: create new content files + add to translations

5. **Analytics Dashboard**
   - Embed Google Analytics
   - Show basic stats in CMS

---

## Timeline Estimate

- **Setup CMS**: 1-2 hours
- **Refactor components**: 3-4 hours
- **Deploy & configure**: 1 hour
- **Training**: 30 minutes

**Total**: ~1 working day (6-8 hours)

---

## Implementation Checklist

1. ✅ Review this plan
2. Create content JSON files with client data
3. Install Decap CMS and create admin panel
4. Refactor pages to read from JSON
5. Deploy to Netlify
6. Enable Netlify Identity
7. Train client on CMS usage
8. Document any client-specific customizations
